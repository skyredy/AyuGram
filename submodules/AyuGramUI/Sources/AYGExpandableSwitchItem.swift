// AYG: a copy of ItemListUI's ItemListExpandableSwitchItem, forked because AyuGram
// needs two things the original does not offer:
//
//   * long-press on a sub-item, which is how AyuGram for Android locks an option so
//     the master switch cannot change it (the footer under the checkboxes says so);
//   * the "N/5" counter drawn in the list's own font instead of a hardcoded bold 14.
//
// Kept as a copy rather than an upstream edit so merges from Telegram stay clean.
// If upstream ever grows a long-press hook, delete this and go back to theirs.
...
// AYG: ItemListUI declares this protocol internal, so the fork cannot see it.
// Redeclared here exactly as PeerInfoUI does for ItemListReactionItem.
...
public enum AYGExpandableSwitchItemNodeType {
    case regular
    case icon
}

public class AYGExpandableSwitchItem: ListViewItem, ItemListItem {
    ...
    let type: AYGExpandableSwitchItemNodeType
    ...
    // AYG: long-press on a sub-item; Android uses it to lock the option.
    let subLongPressAction: ((SubItem) -> Void)?
    public let tag: ItemListItemTag?
    
    public let selectable: Bool = true
    
    public init(presentationData: ItemListPresentationData, systemStyle: ItemListSystemStyle = .legacy, icon: UIImage? = nil, title: String, value: Bool, isExpanded: Bool, subItems: [SubItem], type: AYGExpandableSwitchItemNodeType = .regular, enableInteractiveChanges: Bool = true, enabled: Bool = true, displayLocked: Bool = false, disableLeadingInset: Bool = false, maximumNumberOfLines: Int = 1, noCorners: Bool = false, sectionId: ItemListSectionId, style: ItemListStyle, updated: @escaping (Bool) -> Void, activatedWhileDisabled: @escaping () -> Void = {}, selectAction: @escaping () -> Void, subAction: @escaping (SubItem) -> Void, subLongPressAction: ((SubItem) -> Void)? = nil, tag: ItemListItemTag? = nil) {
        ...
        self.subLongPressAction = subLongPressAction
        self.tag = tag
    }
    
    public func nodeConfiguredForParams(async: @escaping (@escaping () -> Void) -> Void, params: ListViewItemLayoutParams, synchronousLoads: Bool, previousItem: ListViewItem?, nextItem: ListViewItem?, completion: @escaping (ListViewItemNode, @escaping () -> (Signal<Void, NoError>?, (ListViewItemApply) -> Void)) -> Void) {
        async {
            let node = AYGExpandableSwitchItemNode(type: self.type)
    ...
    public func updateNode(async: @escaping (@escaping () -> Void) -> Void, node: @escaping () -> ListViewItemNode, params: ListViewItemLayoutParams, previousItem: ListViewItem?, nextItem: ListViewItem?, animation: ListViewItemUpdateAnimation, completion: @escaping (ListViewItemNodeLayout, @escaping (ListViewItemApply) -> Void) -> Void) {
        Queue.mainQueue().async {
            if let nodeValue = node() as? AYGExpandableSwitchItemNode {
...
private final class AYGSubItemNode: HighlightTrackingButtonNode {
    private let textNode: ImmediateTextNode
    private var checkNode: CheckNode?
    private let separatorNode: ASDisplayNode
    
    private var theme: PresentationTheme?
    private var item: AYGExpandableSwitchItem.SubItem?
    private var action: ((AYGExpandableSwitchItem.SubItem) -> Void)?
    // AYG: long press locks/unlocks the option, so it stays available even while the
    // option is disabled — that is exactly the state the user needs to get out of.
    private var longPressAction: ((AYGExpandableSwitchItem.SubItem) -> Void)?
    ...
    @objc private func longPressed(_ recognizer: UILongPressGestureRecognizer) {
        guard recognizer.state == .began, let item = self.item, let action = self.longPressAction else {
            return
        }
    ...
    @objc private func pressed() {
        // A long press also delivers touchUpInside; swallow the tap that follows it.
    ...
    func update(presentationData: ItemListPresentationData, item: AYGExpandableSwitchItem.SubItem, action: @escaping (AYGExpandableSwitchItem.SubItem) -> Void, longPressAction: ((AYGExpandableSwitchItem.SubItem) -> Void)?, size: CGSize, transition: ContainedViewLayoutTransition) {
...
public class AYGExpandableSwitchItemNode: ListViewItemNode, ItemListItemNode {
    ...
    private var subItemNodes: [AnyHashable: AYGSubItemNode] = [:]
    
    private let activateArea: AccessibilityAreaNode
    
    private var item: AYGExpandableSwitchItem?
    
    public var tag: ItemListItemTag? {
        return self.item?.tag
    }
    
    public init(type: AYGExpandableSwitchItemNodeType) {
    ...
    func asyncLayout() -> (_ item: AYGExpandableSwitchItem, _ params: ListViewItemLayoutParams, _ insets: ItemListNeighbors) -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation) -> Void) {
            ...
            return (ListViewItemNodeLayout(contentSize: contentSize, insets: insets), { [weak self] animation in
                if let strongSelf = self {
                    ...
                    for subItem in item.subItems {
                        ...
                        subItemNode.update(presentationData: item.presentationData, item: subItem, action: item.subAction, longPressAction: item.subLongPressAction, size: subItemSize, transition: subItemNodeTransition)
